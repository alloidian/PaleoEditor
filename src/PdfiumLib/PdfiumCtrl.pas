...
uses
  {$IFDEF FPC} LCLIntf, LCLType, lMessages, {$ELSE} Windows, Messages, {$ENDIF} SysUtils, Classes, Graphics, Controls, Forms, PdfiumCore;
...
var
  ...
  Bits       :Pointer = nil;
begin
  bmi := Default({$IFDEF FPC}LCLType.BITMAPINFO{$ELSE}Windows.TBitmapInfo{$ENDIF});
...
begin
  ...
  if IsPageValid then begin
    ...
    try
      // Draw borders
      FillRect(DrawDC, Classes.Rect(0, 0, Width, FDrawY), Brush.Reference.Handle);                                      // top bar
      FillRect(DrawDC, Classes.Rect(0, FDrawY, FDrawX, FDrawY + FDrawHeight), Brush.Reference.Handle);                  // left bar
      FillRect(DrawDC, Classes.Rect(FDrawX + FDrawWidth, FDrawY, Width, FDrawY + FDrawHeight), Brush.Reference.Handle); // right bar
      FillRect(DrawDC, Classes.Rect(0, FDrawY + FDrawHeight, Width, Height), Brush.Reference.Handle);                   // bottom bar
    ...
    FillRect(DC, Classes.Rect(0, 0, Width, Height), Brush.Reference.Handle);
...
begin
  if FSelectionActive and HandleAllocated then begin
    ...
    if Page <> nil then begin
      ...
      Result := Default(TPdfControlRectArray);
