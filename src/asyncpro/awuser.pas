...
type
  ...
  TApdBaseDispatcher = class
    protected
      ...
      ThreadBoost    : Byte;
      DataSection    : TCriticalSection;       {For all routines}
      OutputSection  : TCriticalSection;       {For output buffer and related data}
      DispSection    : TCriticalSection;       {For dispatcher buffer and related data}
...
var
  PortListSection : TCriticalSection;
...
{$ENDIF}
  ...
  begin
    ...
    DataSection := TCriticalSection.Create;
    OutputSection := TCriticalSection.Create;
    DispSection := TCriticalSection.Create;
      ...
      try
        ...
        if Flags and InHdwFlow <> 0 then begin
          ...
          if (Flags and dcb_OutxDsrFlow <> 0) and
             (bm_COMSTAT_fDsrHold and ComStatus.flag0 > 0) then
            Result := fsDsrHold;

          if (Flags and dcb_OutxCtsFlow <> 0) and
             (bm_COMSTAT_fCtsHold and ComStatus.flag0 > 0) then
            Result := fsCtsHold;
        end;
      finally
        DataSection.Leave;
  ...
  begin
    with DCB do begin
      ...
      try
        ...
        {Set appropriate flow state}
        if (bm_COMSTAT_fXoffHold and ComStatus.Flag0 > 0) then
          if (bm_COMSTAT_fXoffSent and ComStatus.Flag0 > 0) then
            Result := fsXBothHold
          else
            Result := fsXOutHold
        else if (bm_COMSTAT_fXoffSent and ComStatus.Flag0 > 0) then
          Result := fsXInHold;
      finally
        DataSection.Leave;
      end;
...
procedure LockPortList;
begin
  PortListSection.Enter;
end;

procedure UnlockPortList;
begin
  PortListSection.Leave;
end;

initialization
  PortList := TList.Create;
  PortListSection := TCriticalSection.Create;
finalization
  FreeAndNil(PortList);
  PortListSection.Free;
end.
