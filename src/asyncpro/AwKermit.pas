...
uses
  Windows, LCLIntf, LCLType, SysUtils, OoMisc, AwUser, AwTPcl, AwAbsPcl, AdPort;
...
const
  ...
  begin
    with P^ do begin
      ...
      aDataBlock^[12] := Char(kKermitOptions.MaxLongPacketLen div 95);
      aDataBlock^[13] := Char(kKermitOptions.MaxLongPacketLen mod 95);
        ...
        while DIndex < RIndex do begin
          ...
          if kUsingRepeat then
            if Repeating(C, RepeatCnt) then begin
              ...
              aDataBlock^[DIndex+1] := ToChar(Char(RepeatCnt));
              Inc(DIndex, 2);
