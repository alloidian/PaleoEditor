...
uses
  LCLType, Windows, LMessages, Messages, SysUtils, Classes, Graphics, Controls, ExtCtrls,
  Forms, Dialogs, ClipBrd, Menus, OoMisc, AdPort, ADTrmPsr, ADTrmMap, ADTrmBuf;
...
//sg  procedure CMCtl3DChanged(var Msg : TMessage); message CM_CTL3DCHANGED;
      ...
      procedure WMPaint(var Msg : TLMPaint); message WM_PAINT;
...
(*sg  if NewStyleControls and Ctl3D and (FBorderStyle = bsSingle) then begin
...
begin
  ...
  ColCount := FEmulator.Buffer.ColCount;
  tmMarkDeselected(FLButtonRect.Top + 1, FLButtonRect.Left + 1, ColCount);
  for i := succ(FLButtonRect.Top) to pred(FLButtonRect.Bottom) do
    tmMarkDeselected(i, 1, ColCount);
  tmMarkDeselected(FLButtonRect.Bottom + 1, 1, FLButtonRect.Right + 1);
...
{--------}
procedure TAdCustomTerminal.WMPaint(var Msg : TLMPaint);
begin
  PaintHandler(Msg);
end;
{--------}
...
begin
  ...
  case Command of                                                      {!!.04}
    ...
    ecForeColor : begin                                                {!!.04}
      Buffer.ForeColor := GetTColorArg ('Color', 0, clWhite);          {!!.04}
      if GetBooleanArg ('Persistent', 1, True) and                     {!!.04}
         Assigned (Terminal) then                                      {!!.04}
    ...
    ecColor : begin                                                    {!!.04}
      Buffer.ForeColor := GetTColorArg ('Fore', 0, clWhite);           {!!.04}
